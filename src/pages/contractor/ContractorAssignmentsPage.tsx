...
export default function ContractorAssignmentsPage() {
    ...
    const getStatusConfig = (status: string) => {
        switch (status) {
            case 'QUERIED': return { color: 'bg-red-100 text-red-700 border-red-200', icon: AlertTriangle, iconColor: 'text-red-500' };
            case 'PENDING_APPROVAL': return { color: 'bg-yellow-100 text-yellow-700 border-yellow-200', icon: Clock, iconColor: 'text-yellow-500' };
            case 'COMPLETED': return { color: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircle, iconColor: 'text-green-500' };
            default: return { color: 'bg-blue-100 text-blue-700 border-blue-200', icon: Clock, iconColor: 'text-blue-500' };
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <header>
                ...
                <p className="text-sm text-gray-500 mt-1">View and manage your project milestones</p>
            </header>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 text-center">
                    ...
                    <p className="text-xs text-gray-500 uppercase font-medium">Total Projects</p>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 text-center">
                    <p className="text-2xl font-bold text-blue-600">{assignments.filter(a => a.status === 'IN_PROGRESS').length}</p>
                    <p className="text-xs text-gray-500 uppercase font-medium">In Progress</p>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 text-center">
                    <p className="text-2xl font-bold text-red-600">{assignments.filter(a => a.status === 'QUERIED').length}</p>
                    <p className="text-xs text-gray-500 uppercase font-medium">Need Action</p>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 text-center">
            ...
            <div className="space-y-4">
                {sortedAssignments.map(assignment => {
                    ...
                    return (
                        <div
                            key={assignment.id}
                            className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all ${assignment.status === 'QUERIED' ? 'border-red-300 ring-1 ring-red-100' : 'border-gray-200'
                                }`}
                        >
                            {/* Project Header */}
                            <div
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            <h3 className="font-bold text-gray-900 text-lg">{assignment.projectTitle}</h3>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="text-right hidden sm:block">
                                            <p className="text-2xl font-bold text-gray-900">{assignment.overallProgress}%</p>
                            ...
                            {isExpanded && (
                                <div className="border-t border-gray-100 bg-gray-50 p-5">
                                    <h4 className="text-sm font-bold text-gray-700 uppercase mb-4">Milestone Timeline</h4>
                                    <div className="space-y-4">
                                            ...
                                            return (
                                                <div key={milestone.id} className="relative pl-8">
                                                    ...
                                                    {idx < assignment.milestones.length - 1 && (
                                                        <span className="absolute left-3 top-8 w-0.5 h-[calc(100%+8px)] bg-gray-200"></span>
                                                    )}
                                                    {/* Icon */}
                                                    <span className={`absolute left-0 top-1 h-6 w-6 rounded-full flex items-center justify-center ring-4 ring-gray-50 ${milestone.status === 'COMPLETED' ? 'bg-green-100' :
                                                        milestone.status === 'QUERIED' ? 'bg-red-100' :
                                                            milestone.status === 'PENDING_APPROVAL' ? 'bg-yellow-100' : 'bg-blue-100'
                                                        }`}>
                                                        <MIcon className={`h-3.5 w-3.5 ${mConfig.iconColor}`} />
                                                    </span>
                                                    {/* Content */}
                                                    <div className={`bg-white rounded-lg border p-4 ${milestone.status === 'QUERIED' ? 'border-red-200' : 'border-gray-200'
                                                        }`}>
