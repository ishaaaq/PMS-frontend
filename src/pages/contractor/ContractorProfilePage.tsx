                ...
                <div className="px-6 pb-6 -mt-12 relative">
                    <div className="flex flex-col sm:flex-row sm:items-end gap-4">
                        ...
                        <div className="flex-1 pt-2">
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded-full">
                                    ...
                                    <span className="text-sm font-bold text-yellow-700">{profile.overallRating}</span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-500 mt-1">Reg: {profile.registrationNumber}</p>
                        </div>
                        ...
                        <div className="flex gap-6 text-center">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-gray-900">{formatMoney(profile.totalContractValue)}</p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {profile.performanceMetrics.map((metric, i) => (
                    <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
                        ...
                        <p className={`text-xs mt-2 font-medium ${metric.trend === 'stable' ? 'text-gray-400' : 'text-green-600'}`}>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-3">
                        {profile.certifications.map((cert) => (
                            <div key={cert.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-100">
                                <div className="flex items-start gap-3">
                                    <div className={`p-2 rounded-lg ${cert.status === 'VALID' ? 'bg-green-100' : cert.status === 'EXPIRING' ? 'bg-yellow-100' : 'bg-red-100'}`}>
                                        {cert.status === 'VALID' ? (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-3">
                        {profile.portfolio.map((project) => (
                            <div key={project.id} className="p-4 bg-gray-50 rounded-lg border border-gray-100 hover:border-indigo-200 transition-colors cursor-pointer">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex items-center gap-1 bg-yellow-50 px-2 py-0.5 rounded-full">
                                        ...
                                        <span className="text-xs font-bold text-yellow-700">{project.rating}</span>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-200">
                                    <span className="text-sm font-bold text-indigo-600">{formatMoney(project.value)}</span>
