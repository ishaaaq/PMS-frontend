        ...
        return (
            <div className="max-w-6xl mx-auto space-y-6 animate-pulse">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 h-72 bg-gray-200 dark:bg-gray-700 rounded-xl"></div>
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <header className="md:flex md:items-center md:justify-between">
                ...
                <div className="mt-4 flex gap-3 md:mt-0">
                    <button
                        ...
                        className="relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700"
                    >
            ...
            {urgentAction && queriedMilestone && (
                <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-r-lg p-4 flex items-center justify-between shadow-sm">
                    <div className="flex items-center">
                        <AlertTriangle className="h-6 w-6 text-red-500 dark:text-red-400 mr-3 flex-shrink-0" />
                        <div>
                            <p className="text-sm font-bold text-red-800 dark:text-red-300">Action Required: Consultant Query</p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {stats.map((item) => (
                    <div key={item.name} className="glass-card p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{item.name}</p>
                                <p className="mt-1.5 text-2xl font-bold text-gray-900 dark:text-white">{item.stat}</p>
                            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Earnings Chart */}
                <div className="lg:col-span-2 glass-card p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                {/* Recent Activity */}
                <div className="glass-card p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Recent Activity</h3>
                        <button onClick={() => navigate('/contractor/messages')} className="text-xs text-indigo-600 dark:text-indigo-400 font-medium hover:underline">View All</button>
                    </div>
                    <div className="space-y-3">
                        {notifications.slice(0, 4).map(n => (
                            <div
                                key={n.id}
                                className={`p-3 rounded-lg border-l-4 bg-gray-50 dark:bg-gray-800/50 transition-transform hover:scale-[1.01] cursor-pointer ${n.type === 'QUERY' ? 'border-red-400 dark:border-red-500' : n.type === 'PAYMENT' ? 'border-green-400 dark:border-green-500' : n.type === 'APPROVAL' ? 'border-blue-400 dark:border-blue-500' : 'border-gray-300 dark:border-gray-600'
                                    }`}
                            >
                                <div className="flex justify-between items-start">
                                    <h4 className={`font-semibold text-sm ${!n.isRead ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>{n.title}</h4>
            ...
            <div className="glass-card p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => navigate('/contractor/projects')} className="text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:underline flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {assignments.slice(0, 3).map(a => (
                        <div
                            key={a.id}
                            onClick={() => navigate('/contractor/projects')}
                            className={`p-4 rounded-lg border cursor-pointer transition-all hover:shadow-md ${a.status === 'QUERIED' ? 'border-red-200 bg-red-50/50 dark:bg-red-900/10 dark:border-red-900/30' :
                                a.status === 'COMPLETED' ? 'border-green-200 bg-green-50/50 dark:bg-green-900/10 dark:border-green-900/30' : 'border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700'
                                }`}
                        >
                            <div className="flex items-start justify-between mb-2">
                                <h4 className="font-bold text-gray-900 dark:text-white text-sm line-clamp-1">{a.projectTitle}</h4>
                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${a.status === 'QUERIED' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                    a.status === 'COMPLETED' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                        a.status === 'PENDING_APPROVAL' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' :
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="font-bold text-gray-700 dark:text-gray-300">{a.overallProgress}%</span>
                                </div>
                            ...
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-2">{a.milestones.length} milestones</p>
                        </div>
                    ))}
                </div>
            </div>
