    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex justify-between items-center border-b border-gray-200 pb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Consultant Overview</h1>
                    <p className="text-gray-500 text-sm mt-1">Manage verifications and monitor project compliance.</p>
                </div>
                <div className="flex gap-3">
                    ...
                    <button className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 text-sm font-medium transition-colors">
                        Compose Report
                    </button>
                </div>
            </div>
            ...
            <section>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {urgentTasks.map((task) => (
                        <div key={task.id} className="bg-white p-5 rounded-xl shadow-sm border border-l-4 border-gray-100 border-l-indigo-500 hover:shadow-md transition-shadow cursor-pointer">
                            ...
                            <h3 className="font-bold text-gray-900 mb-1">{task.project}</h3>
                            <p className="text-sm text-gray-500 mb-4">Submitted by <span className="font-medium text-gray-700">{task.contractor}</span></p>

                            <button className="w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-sm font-medium transition-colors">
                                Review Submission
                            </button>
                        </div>
                    ))}
            ...
            <section>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {activeProjects.map((project) => (
                        <div key={project.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:border-gray-300 transition-colors">
                            <div className="p-5">
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-bold text-gray-900 truncate pr-2">{project.title}</h3>
                                    <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${project.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                                        }`}>
                                        {project.status}
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 mb-4 flex items-center">
                                    <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded text-[10px] mr-2">{project.location}</span>
